...
class Seq2SeqLSTM(nn.Module):
    ...
    def forward_step(self, input_ids, hidden_states, cell_mem):
        # Embed
        emb = self.tgt_embedding(input_ids) 
        # RNN Step
        out, (hidden_states, cell_mem) = self.decoder(emb, (hidden_states, cell_mem))
        # Output projection
    ...
    def forward(self, x, y):
        ...
        _, (h_n, c_n) = self.encoder(enc_emb)
        ...
        for t in range(decoder_input.size(1)):
            h, c, logits = self.forward_step(decoder_input[:, t].unsqueeze(1), h, c)
    ...
    def predict(self, x, max_len=100):
        ...
        for _ in range(max_len):
            h, c, logits = self.forward_step(y_i, h, c)
